...
**Biggest learning moment:** The adversarial profile test. Setting `mood="sad"` — a value that doesn't exist in the catalog — produced no error, no warning, and no indication that the preference was ignored. The results just looked wrong. That moment made it clear that a system can be logically correct and still mislead users because the failure is invisible. Transparency isn't just about showing *why* a song was recommended — it's also about showing *when* a preference couldn't be satisfied.

**How AI tools helped and when I needed to double-check:** AI helped generate the scoring structure and explained the difference between `sorted()` and `.sort()`. I verified both by running actual output and checking the catalog wasn't mutated between runs. The weight-shift experiment was AI-assisted in setup, but deciding whether results were "better" or just "different" required manual judgment — that can't be automated.

**What surprised me about simple algorithms:** A four-rule formula with hand-picked weights can produce results that genuinely feel like recommendations. When *Library Rain* and *Midnight Coding* surfaced back-to-back for the lofi/chill profile, it felt almost like the system "understood" something. It didn't — it just did arithmetic. The surprise was how easily a human brain pattern-matches math into meaning.

**What I would try next:** A diversity constraint — after scoring, ensure no more than 2 songs from the same genre appear in the top 5. The system currently returns 3 lofi tracks in a row for a lofi user, which is technically correct but not a great listening experience. Real recommenders balance relevance with variety, and that tension is one of the most interesting unsolved problems in the field.
